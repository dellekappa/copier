...
class GlobalState:
    ...
    def render_value(
        ...
        `extra_context` is combined as is with `self.context` when rendering the
        template.
        `extra_answer` are exploded with dpath into the context before rendering
        the template.
        """
        try:
...
def parse_dict_string(string: str) -> dict[str, Any]:
    ...
    Raise a ValueError if parsing failed or
    result isn't a dict.
    """
